...
use tachyonfx::{Effect, EffectManager, Shader, dsl::EffectDsl};
...
impl RuntimeState {
    fn new(spec: TerminalTestSpec) -> Self {
        ...
        state
            .focusable
            .sort_by_key(|index| state.nodes[*index].focus_order);
    ...
    fn install_mount_effects(&mut self) {
        ...
        for motion in &self.spec.motion {
            let trigger_kind = motion
                .trigger
                .get("kind")
                .and_then(Value::as_str)
                .unwrap_or("mount");
            if trigger_kind != "mount" && trigger_kind != "show" {
                continue;
            }
            match compile_motion(motion, self.spec.settings.reduced_motion) {
                Ok(effect) => self
                    .effects
                    .add_unique_effect(motion.effect_id.clone(), effect),
                Err(error) => self
                    .effect_errors
                    .push(format!("{}: {error}", motion.effect_name)),
            }
        }
    }
    ...
    fn trigger_effects(&mut self, trigger: &str, component_id: Option<&str>, key: Option<&str>) {
        for motion in &self.spec.motion {
            ...
            if let Some(component_id) = component_id {
                if motion.component_id != component_id {
                    continue;
                }
            }
            if trigger == "key" {
    ...
    fn apply_timeline(&mut self) {
        for event in &self.spec.scenario.timeline {
            ...
            let Some(component_id) = event.component_id.as_deref() else {
                continue;
            };
            let Some(node) = self.nodes.iter_mut().find(|node| node.id == component_id) else {
                continue;
            };
    ...
    fn handle_key(&mut self, key: KeyEvent) {
        ...
        if ctrl {
            match key.code {
                KeyCode::Char('q') => {
                    ...
                    self.last_event = "Ctrl+Q quit".into();
                    return;
                }
                KeyCode::Char('r') => {
                    self.replay();
                ...
                KeyCode::Char('p') => {
                    ...
                    self.last_event = if self.paused { "paused" } else { "playing" }.into();
                    return;
                }
                KeyCode::Char('+') | KeyCode::Char('=') => {
                    self.speed = (self.speed * 2.0).min(4.0);
                    self.last_event = format!("speed {}x", self.speed);
                    return;
                }
                KeyCode::Char('-') => {
        ...
        if !self.spec.settings.interactive {
            return;
        }
        ...
        if before != after {
            if let Some(id) = before.as_deref() {
                self.trigger_effects("blur", Some(id), None);
            }
            if let Some(id) = after.as_deref() {
                self.trigger_effects("focus", Some(id), None);
            }
        }
    ...
    fn focus_next(&mut self, reverse: bool) {
        ...
        if reverse {
            self.focus_position = if self.focus_position == 0 {
    ...
    fn adjust_selection(&mut self, delta: isize) {
        let Some(index) = self.focused_node_index() else {
            return;
        };
        let node = &mut self.nodes[index];
        if !matches!(
            node.r#type.as_str(),
            "List" | "Menu" | "Select" | "Table" | "Tree"
        ) {
            return;
        }
    ...
    fn edit_focused(&mut self, ch: Option<char>, backspace: bool) {
        ...
        let node = &mut self.nodes[index];
        if !matches!(node.r#type.as_str(), "TextInput" | "TextArea") {
            return;
        }
        let mut value = node
            .props
...
fn compile_motion(motion: &PreviewMotion, reduced: bool) -> Result<Effect, String> {
    let source = if reduced {
        &motion.reduced_motion_dsl
    } else {
        &motion.dsl
    };
    let mut effect = EffectDsl::new()
        .compiler()
        .compile(source)
...
fn node_style(node: &PreviewNode, focused: bool) -> Style {
    ...
    if let Some(color) = parse_color(node.style.get("color")) {
        style = style.fg(color);
    }
    if let Some(color) = parse_color(node.style.get("backgroundColor")) {
...
fn block_for(node: &PreviewNode, focused: bool, title: Option<String>) -> Block<'static> {
        ...
        || focused
    {
        ...
        if let Some(color) = parse_color(node.style.get("borderColor")) {
...
fn scenario_progress(state: &RuntimeState, node: &PreviewNode) -> Option<(f64, String)> {
    if !matches!(node.r#type.as_str(), "ProgressBar" | "Gauge") {
        return None;
    }
    let max = node
        .props
    ...
    let label = if node.r#type == "Gauge" {
        format!(
            "{} {:>3}%",
            value_string(node.props.get("label")),
...
fn render_node(frame: &mut Frame, bounds: Rect, state: &RuntimeState, index: usize) {
    ...
    match node.r#type.as_str() {
        "Screen" | "Box" | "Grid" | "Modal" => {
            let title = if node.r#type == "Modal" {
                Some(value_string(node.props.get("title")))
            } else {
                None
            };
        ...
        "Text" => {
            ...
            let paragraph = Paragraph::new(value_string(node.props.get("content")))
                .style(style)
                .alignment(alignment)
        ...
        "AnsiText" => {
            ...
            let text = source
                .as_bytes()
                .into_text()
                .unwrap_or_else(|_| Text::raw(source));
            frame.render_widget(
                Paragraph::new(text)
                    .block(block_for(node, focused, None))
                    .wrap(Wrap { trim: false }),
                area,
            );
        }
        ...
        "Spinner" => {
            ...
            frame.render_widget(
                Paragraph::new(format!("{} {}", frames[frame_index], label)).style(style),
                area,
            );
        }
        "Button" => {
        ...
        "Tabs" => {
            ...
            let titles = tabs
                .iter()
                .map(|value| Line::from(object_label(value)))
                .collect::<Vec<_>>();
            frame.render_widget(
                Tabs::new(titles)
                    .select(active.min(tabs.len().saturating_sub(1)))
                    .style(style)
        ...
        "StatusBar" | "Breadcrumb" => {
            let key = if node.r#type == "StatusBar" {
                "items"
            } else {
                "items"
            };
            let values = node
                ...
                .cloned()
                .unwrap_or_default();
            let separator = if node.r#type == "Breadcrumb" {
                value_string(node.props.get("separator"))
            } else {
                "  ".into()
            };
            let content = values
        ...
        "Toast" => frame.render_widget(
            Paragraph::new(value_string(node.props.get("message")))
                .style(style)
                .block(block_for(node, focused, None)),
            area,
        ),
        "Separator" => {
            let vertical =
                node.props.get("orientation").and_then(Value::as_str) == Some("vertical");
...
fn render_frame(frame: &mut Frame, state: &mut RuntimeState, delta: StdDuration) {
    ...
    for index in 0..state.nodes.len() {
        render_node(frame, bounds, state, index);
    }
    state
        .effects
        .process_effects(delta.into(), frame.buffer_mut(), bounds);
    if state.spec.settings.show_debug_overlay && bounds.height > 0 {
        ...
        let overlay = format!(
            ...
            state.spec.scenario.name, state.speed, state.elapsed_ms, focused, state.last_event
        );
        let area = Rect::new(
            bounds.x,
        ...
        frame.render_widget(
            Paragraph::new(overlay).style(Style::default().fg(Color::Black).bg(Color::Cyan)),
            area,
        );
    }
    if !state.effect_errors.is_empty() && bounds.height > 1 {
        ...
        frame.render_widget(
            Paragraph::new(format!("TachyonFX: {}", state.effect_errors.join(" | ")))
...
pub fn parse_spec(json: &str) -> Result<TerminalTestSpec, String> {
    let spec: TerminalTestSpec =
        serde_json::from_str(json).map_err(|error| format!("parse terminal test spec: {error}"))?;
    if spec.schema != "syndrid-terminal-test/v1" {
        return Err(format!("unsupported terminal test schema: {}", spec.schema));
    }
    if spec.viewport.width == 0 || spec.viewport.height == 0 {
        return Err("terminal test viewport cannot be zero-sized".into());
    }
    Ok(spec)
}
...
pub fn run_json(json: &str) -> io::Result<()> {
    let spec =
        parse_spec(json).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
    run_spec(spec)
}
...
mod tests {
    ...
    fn renders_real_ratatui_frame_and_tachyonfx() {
        ...
        terminal
            .draw(|frame| render_frame(frame, &mut state, StdDuration::from_millis(100)))
            .expect("draw");
        ...
        assert!(
            !state
                .effect_errors
                .iter()
                .any(|error| error.contains("dissolve"))
        );
    }
    ...
    fn unicode_content_renders_without_panicking() {
        ...
        let mut terminal = Terminal::new(backend).expect("terminal");
        terminal
            .draw(|frame| render_frame(frame, &mut state, StdDuration::ZERO))
            .expect("draw");
    }
    ...
    fn tachyon_dsl_compiles_against_runtime() {
        let motion = PreviewMotion {
            ...
            effect_id: "x".into(),
            effect_name: "X".into(),
            trigger: serde_json::json!({"kind":"mount"}),
            target: serde_json::json!({"kind":"component","componentId":"x"}),
            area: PreviewRect {
                x: 0,
                y: 0,
                width: 10,
            ...
            dsl: "fx::dissolve(300)".into(),
            reduced_motion_dsl: "fx::dissolve(1)".into(),
        };
