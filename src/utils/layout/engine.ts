...
import { stringWidth } from '../rendering/width';
...
export class LayoutEngine {
  ...
  private calculateAutoWidth(node: ComponentNode): number {
    ...
    if (node.type === 'Menu' || node.type === 'List') {
      ...
      if (node.layout.type === 'flexbox' && effectiveDirection === 'row') {
        ...
        items.forEach((item, index) => {
          ...
          const itemWidth = stringWidth(icon) + stringWidth(String(itemData.label ?? '')) + stringWidth(hotkey);
        ...
        items.forEach((item) => {
          ...
          if (node.type === 'Menu') {
            ...
            itemWidth = stringWidth(prefix) + stringWidth(icon) + stringWidth(String(itemData.label ?? '')) + stringWidth(hotkey);
          } else {
            // List items: icon + label + hotkey (with spacing)
            itemWidth = stringWidth(icon) + stringWidth(String(itemData.label ?? '')) + (hotkey ? stringWidth(hotkey) : 0);
          }
    ...
    if (node.type === 'Tabs') {
      ...
      return 1 + tabTexts.reduce((sum: number, text: string) => sum + stringWidth(text) + 4, 0) + 6;
    }
    ...
    switch (node.type) {
      case 'Button': {
        ...
        let contentWidth = stringWidth(label) + padding * 2; // padding on both sides
        ...
        if (iconLeft) {
          if (separated) {
            ...
            const leftSection =
              number !== undefined ? stringWidth(iconLeft) + stringWidth(String(number)) + 1 : stringWidth(iconLeft);
            contentWidth += leftSection + 5; // +5 for " │ " and spaces
          } else {
            // Inline: " icon label "
            contentWidth += stringWidth(iconLeft) + 1; // +1 for space after icon
          }
        }

        // Add right icon
        if (iconRight) {
          contentWidth += stringWidth(iconRight) + 1; // +1 for space before icon
        }
      ...
      case 'Text': {
        ...
        const maxLineLength = Math.max(...lines.map((l) => stringWidth(l)), 10);
