...
export function truncateText(text: string, width: number, ellipsis: string = '…'): string {
  ...
  return sliceByWidth(stripped, width - ellipsisWidth) + ellipsis;
}
...
export function padText(
  ...
  switch (align) {
    case 'center': {
      ...
      return padToWidth(fillChar.repeat(leftPad) + text, width, fillChar);
    }
    case 'right':
      return padToWidth(fillChar.repeat(padding) + text, width, fillChar);
    default: // 'left'
      return padToWidth(text, width, fillChar);
  }
}
