...
import { useComponentStore, useSelectionStore, useThemeStore, useUIStore, useProjectStore } from './stores';
  ...
  useEffect(() => {
    if (!componentStore.root) {
      ...
      if (saved) {
        ...
        if (typeof saved.theme === 'string' && saved.theme in THEMES) {
          useThemeStore.getState().setTheme(saved.theme as keyof typeof THEMES);
        }
        if (saved.project) useProjectStore.getState().setProjectData(saved.project);
        return;
      }
